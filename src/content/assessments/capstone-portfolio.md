...
marking:
  mode: holistic
  description: >
    The capstone is judged as a whole: whether the designed excuse is
...
spec:
  ...
  - the defense explicitly addresses at least one way the excuse could fail under cross-examination
related:
  - assessments/excuse-teardown
  - assessments/case-study-presentation
---

## Instructions

> Design an excuse from scratch, then defend it against the class you've spent
...
Once the excuse is built, the written defense is where the semester's
analytical vocabulary has to do real work. Treat it as a rhetorical brief
...
will read as thin next to a defense that can point to its own sentences.
...
email with a subject line, a transcript of a spoken statement with stage
...
| --- | --- | --- | --- | --- | --- |
| **Originality of the designed excuse** | The scenario and the excuse built for it are clearly the student's own invention, distinct from any earlier assignment, and plausible enough that a real person could give it. | The excuse is original and plausible, with only minor overlap in structure with an earlier assignment. | The excuse is original but leans noticeably on the structure of an earlier assignment, or is only loosely plausible for its scenario. | The excuse is a light rework of an earlier assignment's case with the details changed. | The excuse is substantially the same as work already submitted this semester, or is not genuinely excuse-shaped. |
| **Precision of the semester's vocabulary in the defense** | The defense names its taxonomy category, grammatical devices, and rhetorical strategy precisely, and explains why each specific choice was made rather than merely labelling it. | The defense correctly names and explains most of its rhetorical choices, with at most one term used loosely. | The defense names the main vocabulary correctly but explanation of why each choice was made is thin. | Vocabulary from the semester appears in the defense but is used as decoration rather than to explain an actual choice. | The defense does not use the semester's vocabulary, or uses it incorrectly throughout. |
| **Strength of the pre-empted objection** | The defense identifies the single strongest objection a classmate could raise and answers it directly, conceding what can't be defended rather than talking around it. | The defense identifies a genuine weak point and offers a reasonable answer to it, though not obviously the strongest available objection. | The defense addresses an objection, but a more damaging one is available and left unaddressed. | The objection raised is minor or easily dismissed, avoiding the excuse's real weakness. | No objection is seriously engaged, or the defense treats the excuse as beyond challenge. |
