---
title: Small excuses, big tells
description:
  Week 4's session — workshopping teardown drafts, and what children's excuses
  reveal about how the skill develops
week: 4
date: 2026-03-23
teachers:
  - marisol-quaye
spec:
  - you bring a full draft of your teardown, not notes
  - you can name one moment as a child when you gave an excuse that worked
related:
  - assessments/excuse-teardown
  - lectures/week-04
---

## Before the session

Bring a complete draft of your [teardown](/assessments/excuse-teardown/),
due at the end of the week.

## In the session

Peer workshop on drafts first, then a discussion built around the
developmental psychology in this week's lecture: excuse-making is a skill
children acquire in a fairly predictable order, and the order tells you
something about which parts of an excuse are learned versus instinctive.

## Afterwards

Submit the teardown by Monday noon.
